...
class SuperProxylessNASNets(ProxylessNASNets):
    ...
    def __init__(self, width_stages, n_cell_stages, conv_candidates, stride_stages,
        ...
        for width, n_cell, s in zip(width_stages, n_cell_stages, stride_stages):
            print('\n\n------ width={}, n_cell={}, s={} ------'.format(width, n_cell, s))
            for i in range(n_cell):
                ...
                conv_op = MixedEdge(candidate_ops=candidate_ops)
                if stride == 1 and input_channel == width:
    ...
    def expected_latency(self, latency_model: LatencyEstimator,_batch_size = 8):
        expected_latency = 0
        # first conv
        expected_latency += latency_model.predict('Conv', [224, 224, 3], [112, 112, self.first_conv.out_channels], device = 0)
        # feature mix layer
        # expected_latency += latency_model.predict(
        #     'global_avg_pooling'
        # )
        expected_latency += latency_model.predict(
            'Conv_1', [7, 7, self.feature_mix_layer.in_channels], [7, 7, self.feature_mix_layer.out_channels], device = 2
        )
        # classifier
        expected_latency += latency_model.predict(
            'Logits', [7, 7, self.classifier.in_features], [self.classifier.out_features], device = 2# 1000
        )
        # blocks
        fsize = 112
        for j, block in enumerate(self.blocks):
            ...
            # if j == 6:
            #     communicate_latency =  (fsize*fsize*op.in_channels)*4*_batch_size / (50*125)
            #     print("mixed op communicate_latency on device0 and device1 (ms)",communicate_latency)
            #     # predicted_latency += communicate_latency
            # elif j == 11:
            #     communicate_latency =  (fsize*fsize*op.in_channels)*4*_batch_size / (200*125)
            #     print("mixed op communicate_latency on device1 and device2  (ms)",communicate_latency)
                # predicted_latency += communicate_latency
