...
def proxyless_base(net_config=None, n_classes=1000, bn_param=(0.1, 1e-3), dropout_rate=0):
    ...
    return net
# 基于network config构建了一个没有经过训练的网络，此时我还不确定这个config是经过search获得的还是由一个manual designed model作为search的起点的。倾向于前者
...
class MobileInvertedResidualBlock(MyModule):
    ...
    def __init__(self, mobile_inverted_conv, shortcut):
        ...
        self.shortcut = shortcut # 跳过一层或多层的连接捷径，通常是idnetity但有时候是conv，确保在合并的时候w，h一致
