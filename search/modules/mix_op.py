...
def build_candidate_ops(candidate_ops, in_channels, out_channels, stride, ops_order):
    ...
    name2ops = {
        ...
        # 'Communicate': lambda in_C,out_C, S: ZeroLayer(stride=S),
    }
    # add MBConv layers
    name2ops.update({
...
class MixedEdge(MyModule):
    ...
    # def get_communicate_probs_index(self,communicate_probs):
    #     probs = self.probs_over_ops.data.cpu().numpy()
    #     if len(probs) > 6:
    #         communicate_probs.append(probs[-1])
    #     else:
    #         communicate_probs.append(0)
    #     return communicate_probs
        
    @property
    def chosen_index(self):
        probs = self.probs_over_ops.data.cpu().numpy()
        index = int(np.argmax(probs)) # 选出可能性最大的操作作为最终操作
