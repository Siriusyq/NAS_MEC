...
class RunManager:
    ...
    # noinspection PyUnresolvedvReferences
    def net_latency(self, l_type='gpu8', fast=True, given_net=None,_batch_size=1):
        if 'gpu' in l_type:
        ...
        if l_type == 'mobile':    
            try:
                ...
                predicted_latency += self.latency_estimator.predict(
                    'Conv', [224, 224, 3], [112, 112, net.first_conv.out_channels], device=0
                )
                # feature mix layer
                predicted_latency += self.latency_estimator.predict(
                    'Conv_1', [7, 7, net.feature_mix_layer.in_channels], [7, 7, net.feature_mix_layer.out_channels], device=2
                )
                # classifier
                predicted_latency += self.latency_estimator.predict(
                    'Logits', [7, 7, net.classifier.in_features], [net.classifier.out_features], device=2  # 1000
                )
                # blocks
                fsize = 112
                ...
                #         communicate_probs = mb_conv.get_communicate_probs_index(communicate_probs)
                #     print(communicate_probs)
                # exit()
                for i, block in enumerate(net.blocks):
                    ...
                    block_latency = self.latency_estimator.predict(
                        ...
                        expand=mb_conv.expand_ratio, kernel=mb_conv.kernel_size, stride=mb_conv.stride, idskip=idskip, device=device
                    )
                    if i == 1:
                        communicate_latency =  (fsize*fsize*mb_conv.in_channels)*4*_batch_size / (50*125)
                        print("communicate_latency on device0 and device1 (ms)",communicate_latency)
                        # predicted_latency += communicate_latency
                    elif i == 5:
                        communicate_latency =  (fsize*fsize*mb_conv.in_channels)*4*_batch_size / (200*125)
                        print("communicate_latency on device1 and device2 (ms)",communicate_latency)
                        # predicted_latency += communicate_latency
    ...
    def train_one_epoch(self, adjust_lr_func, train_log_func):
        ...
        # 减小调试时间
        datalen = len(self.run_config.train_loader)
        datause = datalen/50
        for i, (images, labels) in enumerate(self.run_config.train_loader[0:datause-1]):
    ...
    def train(self, print_top5=False):
        ...
        nBatch = self.train_one_epoch.datause
